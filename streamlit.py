...
def train_page():
  ...
  if st.button("Train Model"):
    ...
    st.success("Time: "+ str(round(time()-start,1)))
...
def predict_page():
  ...
  if st.button("Predict"):
      ...
      class_map = { i: chr(i + 65) for i in range(26) } #Binary thành alphabet
  ...
  if image is not None:
    ...
    with col2:
      ...
      image1 = cv2.resize(image1, dsize=(32, 32))  # Adjust dimensions
      image1 = image1.astype("float32") / 255.0      
      image1 = img_to_array(image1)

      image1 = np.expand_dims(image1, axis=0)
      # Make prediction using the trained model
      prediction = model[0].predict(image1)
      class_map = { i: chr(i + 65) for i in range(26) } #Binary thành alphabet
      top_k_predictions = np.argsort(prediction[0])[-5:][::-1] 
      top_k_proba = prediction[0][top_k_predictions] 
    ...
    with col3:
      st.header("Inverted grayscale")
      image2 = np.invert(image.convert("L"))
      st.image(image2)
      image2 = np.array(image2)
      image2 = cv2.resize(image2, dsize=(32, 32))  # Adjust dimensions
      image2 = image2.astype("float32") / 255.0      
      image2 = img_to_array(image2)

      image2 = np.expand_dims(image2, axis=0)
      # Make prediction using the trained model
      prediction = model[0].predict(image2)
      class_map = { i: chr(i + 65) for i in range(26) } #Binary thành alphabet
      top_k_predictions = np.argsort(prediction[0])[-5:][::-1] 
      top_k_proba = prediction[0][top_k_predictions] 
